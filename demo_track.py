...
from utils.visualize import plot_tracking, plot_det, plot_circle, plot_polygon
from tracker.byte_tracker import BYTETracker
from tracking_utils.timer import Timer
from utils.boxes import postprocess, xyxy2circle, circle2innerxyxy, xywh2xyxy, rbox2poly
import argparse
import os
import time
from models.common import DetectMultiBackend
from utils.augmentations import letterbox
from utils.general import non_max_suppression, non_max_suppression_obb, scale_polys, scale_coords
...
class Predictor(object):
    ...
    def inference(self, img, timer):
        ...
        with torch.no_grad():
            ...
            outputs = non_max_suppression_obb(outputs, self.confthre, self.nmsthre, multi_label=True)
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    save_folder = os.path.join(
        vis_folder, save_name, timestamp
    )
    os.makedirs(save_folder, exist_ok=True)
    if args.demo == "video":
        save_path = os.path.join(save_folder, args.path.replace('\\', '/').split("/")[-1])
    else:
    ...
    while True:
        ...
        if ret_val:
            ...
            # 画原始模型预测框
            bboxes = copy.deepcopy(outputs[0][:, :5])
            pred_poly = rbox2poly(bboxes)
            pred_poly = scale_polys(img_info['resize_sz'], pred_poly, img_info['raw_img'].shape)
            img_info['raw_img'] = plot_polygon(img_info['raw_img'], pred_poly, img_info, args)

            # 画内切圆的内接矩形框
            # try:
            #     bboxes = circle2innerxyxy(xyxy2circle(copy.deepcopy(outputs[0][:,:4])))
            #     bboxes = xywh2xyxy(bboxes)
            #     bboxes = scale_coords(img_info['resize_sz'], bboxes, img_info['raw_img'].shape).round()
            #     img_info['raw_img'] = plot_det(img_info['raw_img'], bboxes, img_info, args)
            # except Exception as e:
            #     print(e)

            # try:
            #     outputs[0][:,:4] = xywh2xyxy(circle2innerxyxy(xyxy2circle(outputs[0][:,:4])))
            # except:
            #     pass

            # for i, det in enumerate(outputs):
            if outputs[0] is not None:
                online_targets = tracker.update(outputs[0], img_info, [img_info['height'], img_info['width']],
                                                args.tsize, args)
                online_tlwhs = []
                online_ids = []
                online_scores = []

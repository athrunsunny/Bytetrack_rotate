...
import time
from utils.boxes import xyxy2cxcywh, rbox2poly
...
def iou_distance(atracks, btracks, norm=False):
    """
    Compute cost based on IoU
    ...
    if norm:
        ...
        atlbrs = rbox2poly(xyxy2cxcywh(atlbrs)) if len(atlbrs) != 0 else np.array([])
        btlbrs = rbox2poly(xyxy2cxcywh(btlbrs)) if len(btlbrs) != 0 else np.array([])

        _ious = bbox_overlaps(atlbrs, btlbrs)[0]
